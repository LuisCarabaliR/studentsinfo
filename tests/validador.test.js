...
describe('Validación de archivos JSON', () => {
    ...
    // Si no hay archivos modificados, imprime un mensaje y agrega una prueba "dummy"
    if (changedFiles.length === 0) {
        console.log('No se modificaron archivos info.json en public/data');
        
        // Asegúrate de que haya al menos una prueba que se ejecute
        test('No hay archivos para validar', () => {
            expect(true).toBe(true); // Prueba dummy que siempre pasa
        });
        
        return; // Finaliza la ejecución de las pruebas
    }
